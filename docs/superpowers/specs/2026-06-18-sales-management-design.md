...
### 3.1 損益・損益分岐点（BEP）のルール

- **売上（税抜）** = 研修費用 + 交通費 + その他（消費税は含めない）。
- **変動費** = 講師料（外部委託講師の費用）。社員講師の人件費は固定費に含める。
- **固定費** = 年度ごとに「月額固定費」を1つ設定し、年間固定費 = 月額 × 12。
- **限界利益** = 売上 − 変動費 / **限界利益率** = 限界利益 ÷ 売上（= 1 − 変動費率）。
- **損益分岐点売上高（BEP）** = 年間固定費 ÷ 限界利益率。
- **営業利益** = 売上 − 変動費 − 固定費。
- **安全余裕率** = (現売上 − BEP) ÷ 現売上。**BEP達成率** = 現売上 ÷ BEP。
- **粗利** = 売上 − 変動費（講師料）（= 限界利益と同義）。**粗利率** = 粗利 ÷ 売上。月別に推移を出す。
- **前年同月比** = 当年度の各月売上 ÷ 前年度同月売上 − 1（%）。前年度データが無い月は「前年データなし」と表示。
- グラフ:
  1. **年間BEP図**: X軸=売上高、「売上線(y=x)」と「総費用線(y=年間固定費＋変動費率×売上)」の交点=BEP。現在の実績売上点を重ねる。
  2. **月次累計の黒字転換点**: 月別に「累計売上」と「累計総費用（累計固定費＋累計変動費）」を折れ線で描き、交差した月＝黒字転換月。
  3. **前年同月比**: 当年度月別売上（棒）に前年度同月（線）を重ね、増減%を併記。
  4. **粗利率推移**: 月別の粗利率を折れ線で表示。
  5. **得意先トップ5**: 年度の税抜売上が多い順に上位5社（金額・シェア率・前年比があれば併記）。
...
### 5.3 settings（年度設定 / 損益分岐点用）
| 項目 | 型 | 備考 |
|---|---|---|
| fiscal_year | int PK | 年度（例: 2026） |
| monthly_fixed_cost | int default 0 | 月額固定費。年間固定費 = ×12 |
| updated_at | datetime | |

年度を切り替えると、その年度の月額固定費で BEP を計算する。未設定なら 0（=固定費未入力の注意表示）。
...
- `GET /api/summary/pl?fiscal_year=` … 損益・BEP（売上/変動費/固定費/限界利益率/BEP/営業利益/安全余裕率＋年間BEP図と月次累計用の系列データ／月別粗利率／得意先トップ5）
- `GET /api/summary/monthly?fiscal_year=` の応答に **前年同月（prev_year 系列）** を含める（前年度データが無い月は null）
- `GET /api/payments?status=unpaid&fiscal_year=` … 入金管理
- `GET /api/masters/{clients|instructors|agencies}` / `POST` / `PUT` / `DELETE`
- `GET /api/settings/{fiscal_year}` / `PUT /api/settings/{fiscal_year}` … 月額固定費の取得・更新
...
1. **ダッシュボード**: 年度切替、月別売上グラフ（棒）、年度合計、未入金合計、今月の入金予定一覧、**BEP達成率（売上÷損益分岐点）KPI と営業利益**。
...
6. **損益・損益分岐点（収支総括）**: 年度のトータルを1画面で。KPI（税抜売上／変動費／固定費／限界利益・限界利益率／営業利益／損益分岐点売上高／BEP達成率・安全余裕率）＋ **年間BEP図**（売上線×総費用線の交点）＋ **月次累計の黒字転換点**折れ線 ＋ **前年同月比**（当年度棒＋前年度線・増減%）＋ **粗利率推移**（折れ線）＋ **得意先トップ5**（金額・シェア率）。月額固定費はこの画面から設定。
7. **入金管理**: 未入金一覧、入金予定（月別）、「入金済みにする」操作。
8. **マスタ管理**: 企業/講師/代理店の一覧・追加・編集・無効化。
9. **Excel連携**: 既存xlsxの初期取り込み（1回）＋ Excelエクスポート。
...
- テスト: 金額計算（tax/billing）・売上月導出・年度導出・集計（月別/年間/軸別）・損益分岐点（限界利益率/BEP/営業利益/月次累計の黒字転換）・取り込み変換のユニットテストを backend に置く（pytest）。

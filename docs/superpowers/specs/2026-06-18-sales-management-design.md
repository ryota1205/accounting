...
- `GET /api/summary/pl?fiscal_year=` … 損益・BEP（売上/変動費/固定費/限界利益率/BEP/営業利益/安全余裕率＋年間BEP図と月次累計用の系列データ）
...
6. **損益・損益分岐点（収支総括）**: 年度のトータルを1画面で。KPI（税抜売上／変動費／固定費／限界利益・限界利益率／営業利益／損益分岐点売上高／BEP達成率・安全余裕率）＋ **年間BEP図**（売上線×総費用線の交点）＋ **月次累計の黒字転換点**折れ線。月額固定費はこの画面から設定。
